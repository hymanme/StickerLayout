...
import com.hymane.xstickerlayout.xtag.easing.Cubic;
import com.hymane.xstickerlayout.xtag.easing.Easing;
import com.hymane.xstickerlayout.xtag.graphics.FastBitmapDrawable;
import com.hymane.xstickerlayout.xtag.utils.IDisposable;
...
public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    protected Handler mStickerHandler = new Handler();
    ...
    public float getTopPadding() {
        ...
        return (height - drawable.getIntrinsicHeight()) * 0.5f;
    }
    ...
    protected void getProperBaseMatrix(Drawable drawable, Matrix matrix) {
        ...
        if (w > viewWidth || h > viewHeight) {
            ...
            mMinZoom = 1;
            mSuppMatrix.setScale(widthScale, heightScale);

            float tw = (viewWidth - w * widthScale) / 2.0f;
            float th = (viewHeight - h * heightScale) / 2.0f;
            matrix.postTranslate(tw, th);
            mSuppMatrix.setTranslate(tw, th);
        } else {
            widthScale = viewWidth / w;
            heightScale = viewHeight / h;
            float scale = Math.max(widthScale, heightScale);
    ...
    protected float getTranslationX(Matrix matrix) {
        return getValue(matrix, Matrix.MTRANS_X);
    }

    protected float getTranslationY(Matrix matrix) {
    ...
    public float getTranslateX() {
        return getTranslationX(mSuppMatrix);
    }

    public float getTranslateY() {
        return getTranslationY(mSuppMatrix);
    }
    ...
    public void scrollBy(float distanceX, float distanceY, final double durationMs) {
        final double dx = distanceX * 2.0;
        final double dy = distanceY * 2.0;
